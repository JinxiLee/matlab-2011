...
Is_ftemp=fft(Is);
Is_ftemp1=Is_ftemp(1:round(length(Is_ftemp)/2));
Is_ftemp2=Is_ftemp((round(length(Is_ftemp)/2)+1):end);
Is_ftemp=[Is_ftemp1;zeros(length(Is_ftemp)*3,1); Is_ftemp2];
Is=real(ifft(Is_ftemp));

Ip_ftemp=fft(Ip);
Ip_ftemp1=Ip_ftemp(1:round(length(Ip_ftemp)/2));
Ip_ftemp2=Ip_ftemp((round(length(Ip_ftemp)/2)+1):end);
Ip_ftemp=[Ip_ftemp1;zeros(length(Ip_ftemp)*3,1); Ip_ftemp2];
Ip=real(ifft(Ip_ftemp));
...
Starting_pixel=10000;

Ending_pixel=13000;

saparation_index=11600;
...
Ip_interp=interp1(position,Ip,position);
...
for j=1:length(ratiop)
    for k=1:20  
Ip_sub=(Ip_interp-circshift(CS_real,[0 10-goodShiftp])/max(CS_real)*Ip_peakvalue*ratiop(j));  %this 0.9 can be critical...

if max(Ip_sub(1:saparation_index))<tempmaxp
    tempmaxp=max(Ip_sub(1:saparation_index));
    goodRatiop=ratiop(j);
    goodShiftp=k;
end
    end
...
%% to use good ratio
        ...
        Ip_sub=Ip_interp;%-circshift(CS_real,[0 10-goodShiftp])/max(CS_real)*Ip_peakvalue*goodRatiop;  %this 0.9 can be critical...
        Subp=circshift(CS_real,[0 10-goodShiftp])/max(CS_real)*Ip_peakvalue*goodRatiop;

...
Wavelength_Considered_Min=545;          %nm
Wavelength_Considered_Max=555;
...
Signal_1((round(length(Signal_1)/2)+1):end);
Signal_2((round(length(Signal_2)/2)+1):end);
Signal_3((round(length(Signal_3)/2)+1):end);
...
Spectrum_3=(ifft(Signal_3));        % With phase Now
...
% initial guesses:
    ...
    Thickness_Pitch=0.01E-6;
    Efficiency_Pitch=0.01;
    ...
    %Efficiency: 1
        ...
        Efficiency_2=0.8;
        Efficiency_3=0.8;
        Efficiency=Efficiency-10*Efficiency_Pitch;
    ...
    elseif Load_Previous_Result == 1
        ...
        Efficiency=importdata('Efficiency_1.txt');
    ...
    while (q < Number_of_loop)                                                    % q: Loop 2, for Efficiency_1, Efficiency_2, Efficiency_3, Thickness, Thickness_0
        ...
        for j=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
            ...
            if j <= ((Frequency_Considered_Min_Index-Frequency_Center_Index)+1)
                index_now=Frequency_Center_Index+j-1;
            elseif j > ((Frequency_Considered_Min_Index-Frequency_Center_Index)+1)           %direction reverse!!
                index_now=-j+((Frequency_Considered_Min_Index-Frequency_Center_Index)+1)+Frequency_Center_Index;
            end
            
            % About the conditions
            r1=(n1-(n_temp+i*k_temp))./(n_temp+n1+i*k_temp);
            r2=((n_temp+i*k_temp)-n2(index_now))./((n_temp+i*k_temp)+n2(index_now));   
            d=exp(i*2*pi.*Frequency(index_now)/C.*(-1*n_temp+i*k_temp).*Thickness);   %猔種! -1*n!
            d0=exp(i*2*pi.*Frequency(index_now)/C.*Thickness_0);   %猔種! -1*n!
            % Operand_1
            Spectrum_1_Temp=(abs(r1).^2).*(Spectrum_Original(index_now))+(abs(r2.*(d.^2)).^2).*(Spectrum_Original(index_now))+2*Efficiency_1*(r1.*r2.*Spectrum_Original(index_now).*(d.^2));
            % Operand_2
            Spectrum_2_Temp=2*Efficiency_2*(r_BK7(index_now).*r1.*Spectrum_Original(index_now).*d0.^2);                       %弧琌cosine
            % Operand_3
            Spectrum_3_Temp=2*Efficiency_3*(r_BK7(index_now).*r2.*Spectrum_Original(index_now).*(d0.*d).^2);
            % Merit Function
            ...
            Merit=(abs(Spectrum_2_Temp-Spectrum_2(index_now)).^2)+(abs(Spectrum_3_Temp-Spectrum_3(index_now)).^2);
            ...
            Error_Total_2_Temp= Error_Total_2_Temp+((Spectrum_2_Temp(index_1(index_2_temp), index_2)-Spectrum_2(index_now)).^2);
            Error_Total_3_Temp= Error_Total_3_Temp+((Spectrum_3_Temp(index_1(index_2_temp), index_2)-Spectrum_3(index_now)).^2);
        ...
        if Error_Difference >= 0                    % Start to diverge
            ...
            elseif Current_Scanning_Parameter == 5
        ...
        elseif Current_Scanning_Parameter == 2
            Thickness=Thickness+Direction*Thickness_Pitch;
        elseif Current_Scanning_Parameter == 3
            Efficiency=Efficiency+Direction*Efficiency_Pitch;
            Efficiency_1=Efficiency_1+Direction*Efficiency_Pitch;
        elseif Current_Scanning_Parameter == 4
            Efficiency_2=Efficiency_2+Direction*Efficiency_Pitch;
        elseif Current_Scanning_Parameter == 5
        ...
        Efficiency_2=Efficiency;
        Efficiency_3=Efficiency;
    end

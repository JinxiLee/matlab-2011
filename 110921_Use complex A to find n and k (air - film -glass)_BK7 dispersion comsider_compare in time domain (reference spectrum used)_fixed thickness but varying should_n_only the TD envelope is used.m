...
for q=1:length(n_should)
    ...
    Signal_envelope_calculated=abs(ifft(Reference_f.*r1_calculated+Reference_f.*(d_calculated.^2).*r2_calculated));
    
    for rr=1:length(A)
    
        value_total=sum(abs(Signal_envelope-A(rr)*Signal_envelope_calculated).^2);
...
Signal_check=abs(ifft(Reference_f.*r1_check+Reference_f.*(d_check.^2).*r2_check));
...
plot(position,Signal_envelope,position,A(A_finalindex)*Signal_check);

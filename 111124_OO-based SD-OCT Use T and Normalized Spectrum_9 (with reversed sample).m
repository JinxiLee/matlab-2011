...
for p=1:Number_of_Loop
    ...
    for j=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
        ...
        Merit=((angle(Spectrum_Temp)-(angle(Spectrum(index_now)))).^2);%((Spectroscopy_Temp-(Spectroscopy(index_now))).^2)++((Spectroscopy_Temp-(Spectroscopy(index_now))).^2); %(abs(abs(Spectrum_Temp)-abs(Spectrum(index_now))).^2)+
...
% For Spectroscopy    
    ...
    for j=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
        ...
        Spectroscopy_Temp=abs((t_AN100(index_now)).*(t1.*t2.*d)./(1+r1_r.*r2.*(d.^2))).^2;
        %T_Temp=abs((t_AN100(index_now).^2).*(t1.*t2.*d)./(1+r1_r.*r2.*(d.^2))).^2;
        Merit=((Spectroscopy_Temp-(Spectroscopy(index_now))).^2);%+((real(Spectrum_Temp)-(real(Spectrum(index_now)))).^2);%+((Spectroscopy_Temp-(Spectroscopy(index_now))).^2); %(abs(abs(Spectrum_Temp)-abs(Spectrum(index_now))).^2)+
...
plot(Wavelength_micron,real(Spectrum),Wavelength_micron,real(Spectrum_Check));

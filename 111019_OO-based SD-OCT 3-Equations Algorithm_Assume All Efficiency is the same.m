...
% initial guesses:
    ...
    while (q < Number_of_loop)                                                    % q: Loop 2, for Efficiency_1, Efficiency_2, Efficiency_3, Thickness, Thickness_0
        ...
        for j=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
            ...
            % Operand_1
            Spectrum_1_Temp=(abs(r1).^2).*(Spectrum_Original(index_now))+(abs(r2.*(d.^2)).^2).*(Spectrum_Original(index_now))+2*Efficiency_1*real(r1.*r2.*Spectrum_Original(index_now).*(d.^2));
            % Operand_2
            Spectrum_2_Temp=2*Efficiency_2*real(r_BK7(index_now).*r1.*Spectrum_Original(index_now).*d0.^2);                       %弧琌cosine
            % Operand_3
            Spectrum_3_Temp=2*Efficiency_3*real(r_BK7(index_now).*r2.*Spectrum_Original(index_now).*(d0.*d).^2);
            % Merit Function

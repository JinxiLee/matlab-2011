...
data_o=importdata('D:\110228_lateral (200sum10 -0.35 to 0 0.005)LCD 30X.txt');
data(1:fix(size(data_o,1)/5),1:size(data_o,2))=0;
for j=1:fix(size(data_o,1)/5)
    data(j,:)=data_o(5*j,:);
end
%data=data_o;
clear data_o;
...
%%  Interfered PSF FWHM
space_min_for_inter_peak_FD=2;  %要求跟TD差不多
...
maxs_2d(10000,1:279)=0;
for j=1:279
    maxs_2d(:,j)=maxs(j);
end
imagesc(10*log10(CS_envelope(1:10000,:)./maxs_2d),'xdata',[0:347.5/279:347.5],'ydata',[0:173.3864921242903/10000:173.3864921242903]); figure(gcf)

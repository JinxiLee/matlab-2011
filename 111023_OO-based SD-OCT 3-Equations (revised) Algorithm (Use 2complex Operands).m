...
% initial guesses:
    ...
    while (q <= Number_of_loop)                                                    % q: Loop 2, for Efficiency_1, Efficiency_2, Efficiency_3, Thickness, Thickness_0
        ...
        for j=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
            ...
            % Operand_2
            Spectrum_2_Temp=2*Efficiency_2*(Spectrum_Original(index_now).*r_BK7(index_now).*r1.*d0.^2);                       %弧琌cosine
            % Operand_3
            Spectrum_3_Temp=2*Efficiency_3*(Spectrum_Original(index_now).*r_BK7(index_now).*t1.*t1_r.*r2.*(d0.*d).^2);
            % Merit Function

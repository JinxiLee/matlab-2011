...
data_Bscan=importdata('D:\111224\111224_SMALL SAMPLE 1000micron ave10 2.txt');
ref_Bscan=importdata('D:\111224\111224_mirror 1000micron ave10.txt');
...
profile_calibrated=profile-profile_ref*ratio(b);
    for j=1:size(profile_calibrated,1)
        ...
        if (value_max(j)<0.5*value_max_mean) && (j > 1)
           %profile_tilted(j)=profile_tilted(j-1) ;
           profile_tilted(j)=0;
        end
    end
    for j=1:size(profile_calibrated,1)
        if (profile_tilted(j)==0) && (j > 1)
           index_left=find((profile_tilted(1:j)>0),1,'last');
           index_right=find((profile_tilted(j:end)>0),1,'first')+j-1;
           profile_tilted(j)=profile_tilted(index_left)+(profile_tilted(index_right)-profile_tilted(index_left))*(j-index_left)/(index_right-index_left);
        end
    end

...
%if (space_FD(j)<period/10) && (space_FD(j)>-period/10)
%cosinefunction(j)=100*cos(2*pi*(space_FD(j)+period/10)/period);
%sinefunction(j)=100*sin(2*pi*(space_FD(j)+period/10)/period);
%end
%end
...
for j=1:round(length(PSF)/2)
    PSF_int(j)=sum(PSF(max(1,(PSF_max_index-j)):min(length(PSF),(PSF_max_index+j))));
end    

plot(space_FD(1:length(PSF_int)),PSF_int);
...
%cosinefunction_temp=circshift(cosinefunction,[0 (PSF_max_index-j)]).*PSF;
%sinefunction_temp=circshift(sinefunction,[0 (PSF_max_index-j)]).*PSF;
...
%for j=1:round(length(PSF)/2)
%    PSF_int(j)=sum(PSF((PSF_max_index-j):(PSF_max_index+j)));
%end    

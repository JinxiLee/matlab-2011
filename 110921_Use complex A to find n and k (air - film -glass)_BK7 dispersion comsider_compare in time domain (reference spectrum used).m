...
for kk=1:50
    ...
    Referece=importdata(sprintf('%i',kk));
    
    Referece(4500:end)=Referece(4500);
    
    [signal_maxvalue ref_maxindex]=max(Referece,[],1);
...
Referece=importdata(sprintf('%i',req_ref_kk));

Referece(4500:end)=Referece(4500);

[maxvalue maxindex]=max(Referece,[],1);
needshift=-maxindex;

Referece=circshift(Referece,needshift);
...
Referece_temp_f=fft(Signal);

Referece_temp_f(1:Starting_pixel_f)=0;
Referece_temp_f((length(Referece_temp_f)-Starting_pixel_f+1):length(Referece_temp_f),:)=0;

Referece=real(ifft(Referece_temp_f));

Referece(pixel_2:pixel_3,:)=0;

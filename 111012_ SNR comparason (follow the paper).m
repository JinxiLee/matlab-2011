...
ti=10.^logt;                               %for a A-scan

h=6.626E-34;        %plank constant
c=3E8;              %light speed
d_freq=(c/(0.65E-6))-(c/(0.85E-6));

SigSD=(2*qe*((Pr.*Ps).^0.5)/h/(c/(wavelength*10^-6)))*ti/2/N;        %for one pixel after FFT, N is timed back asuumed FFT
...
SNRdark_SD=10*log10((SigSD.^2)./(dark_SD2))';
SNRdark_TD=10*log10((SigTD.^2)./(dark_TD2))';
SNRexcess_SD=10*log10((SigSD.^2)./(excess_SD2))';
...
plot(logt,SNRtotal_SD,logt,SNRtotal_TD,logt,SNRshut_SD,logt,SNRshut_TD);

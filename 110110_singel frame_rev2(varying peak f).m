...
data_array_o=importdata('D:\110110_ali3_2.txt');

N_zero=1;

%% Zero filling

fft_data_array=fft(data_array_o);
zeros(1:(N_zero-1)*length(fft_data_array))=0;
zeros=zeros';
fft_data_array_p=[fft_data_array(1:round(length(fft_data_array)/2)); zeros; fft_data_array((round(length(fft_data_array)/2)+1):end)];
data_array=ifft(fft_data_array_p);
...
index_1=N_zero*index_1_o;
index_2=N_zero*index_2_o;
...
found_peak_wavelength_c=lambda_o(dc_max_index);

found_peak_wavelength=(found_peak_wavelength_c-10):1:(found_peak_wavelength_c+10);
FWHM_FD(1:length(found_peak_wavelength))=0;


for j=1:length(found_peak_wavelength)

incidence_angle=asin(found_peak_wavelength(j)/2/grating_pitch);     %假設是照此角度入射,rad

theta_1=asin((wavelength_1-0.5*found_peak_wavelength(j))/grating_pitch);
theta_2=asin((wavelength_2-0.5*found_peak_wavelength(j))/grating_pitch);

Q=(tan(theta_1-incidence_angle)-tan(theta_2-incidence_angle))/(index_1-index_2);      %roughly flens/pixel size
%%tan!
lambda=grating_pitch*sin((atan((pixel-dc_max_index)*Q)+incidence_angle))+found_peak_wavelength(j)/2;   %atan!
...
%% data related
S0=data_array-mean(data_array(1:100));
...
CS_carrier=real(CS);
CS_envelope=abs(CS);                                        %data 1 to record (need decimate? maybe cut is better)
%% 

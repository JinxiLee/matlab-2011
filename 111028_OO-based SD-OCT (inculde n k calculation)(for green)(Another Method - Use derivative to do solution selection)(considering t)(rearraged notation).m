...
Data=importdata('111031_New Green (400mA 3ms 100ave) near c 2.txt');
...
pixel_1=295;               % the end of upper (the larger peak)
...
for q=1:length(n_should)
    ...
    n_o=(n_should(q)-0.3):0.001:(n_should(q)+0.3);
    k_o=0:0.001:0.1;
    k_o=k_o';
    ...
    A(1:length(Frequency_Final),1)=0;
    ...
    for p=1:(Ending_pixel_f_considered-Starting_pixel_f_considered+1)            %start from center_frequency_index, to Ending_pixel_f, back to center_frequency_index, finally Starting_pixel_f_considered
        ...
        t1=((-1)*n1)./((-1)*n_temp+n1+i*k_temp);
        t1_r=((-1)*n_temp+i*k_temp)./((-1)*n_temp+n1+i*k_temp);
        r1=((-1)*n1-((-1)*n_temp+i*k_temp))./((-1)*n_temp+(-1)*n1+i*k_temp);
        r2=(((-1)*n_temp+i*k_temp)-(-1)*n2(index_now))./(((-1)*n_temp+i*k_temp)+(-1)*n2(index_now));    
        d=exp(i*2*pi.*Frequency_Final(index_now)/c.*((-1)*n_temp+i*k_temp).*thickness_temp);   %注意! -1*n!
        ...
        A(index_now)=A_temp(index_1(index_2_temp), index_2);    %index1,index2=k,n
...
t1_check=((-1)*n1)./((-1)*n_final+(-1)*n1+i*k_final);
t1_r_check=((-1)*n_final+i*k_final)./((-1)*n_final+n1+i*k_final);
r1_check=((-1)*n1-((-1)*n_final+i*k_final))./((-1)*n_final+(-1)*n1+i*k_final);
r2_check=(((-1)*n_final+i*k_final)-(-1)*n2)./(((-1)*n_final+i*k_final)+(-1)*n2);    
d_check=exp(i*2*pi.*Frequency_Final/c.*((-1)*n_final+i*k_final).*thickness_temp);   %注意! -1*n!

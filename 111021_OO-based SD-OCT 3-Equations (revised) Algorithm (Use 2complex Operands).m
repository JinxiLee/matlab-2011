...
% initial guesses:
    ...
    Thickness_Pitch=0.02E-6;
    Efficiency_Pitch=0.02;
    ...
    while (q < Number_of_loop)                                                    % q: Loop 2, for Efficiency_1, Efficiency_2, Efficiency_3, Thickness, Thickness_0
        ...
        for j=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
            ...
            % Operand_2
            Spectrum_2_Temp=2*Efficiency_2*real(Spectrum_Original(index_now).*r_BK7(index_now).*r1.*d0.^2);                       %弧琌cosine
            % Operand_3
            Spectrum_3_Temp=2*Efficiency_3*real(Spectrum_Original(index_now).*r_BK7(index_now).*t1.*t1_r.*r2.*(d0.*d).^2);
            % Merit Function
            ...
            Merit=((Spectrum_2_Temp-Spectrum_2(index_now)).^2)+((Spectrum_3_Temp-Spectrum_3(index_now)).^2);
            ...
            Error_Total_Temp=Error_Total_2_Temp+Error_Total_3_Temp;
        end
...
Spectrum_2_check=2*Efficiency_2*real(Spectrum_Original.*r_BK7.*r1_check.*d0_check.^2);                       %弧琌cosine
Spectrum_3_check=2*Efficiency_3*real(Spectrum_Original.*r_BK7.*t1_check.*t1_r_check.*r2_check.*(d0_check.*d_check).^2);

plot(Frequency,Spectrum_1,Frequency,Spectrum_1_check);

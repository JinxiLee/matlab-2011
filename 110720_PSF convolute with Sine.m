...
cosineefunction(1:length(space_FD))=0;
sineefunction(1:length(space_FD))=0;
for j=1:length(space_FD)
if (space_FD(j)<period/10) && (space_FD(j)>-period/10)
cosineefunction(j)=100*cos(2*pi*(space_FD(j)+period/10)/period);
sineefunction(j)=100*sin(2*pi*(space_FD(j)+period/10)/period);
end
end
...
result_C=conv(cosineefunction,PSF);
result_S=conv(sineefunction,PSF);
...
angle=atan(result_S./result_C);
%%
birefringence=diff(angle)*0.56/4/pi./diff(space_FD);


dlmwrite('D:\result_S.txt',result_S,'delimiter','\t','newline','pc');
dlmwrite('D:\result_C.txt',result_C,'delimiter','\t','newline','pc');

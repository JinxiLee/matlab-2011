...
value_total_2(1:Number_of_Loop)=0;
    ...
    n_o=(n_should(q)-0.4):0.0001:(n_should(q)+0.4);
    k_o=0:0.0001:0.05;
    %k_o=k_o';
...
% For Spectroscopy    
    Spectroscopy_Check(1:length(Spectrum))=0;
    for j=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
        ...
        Merit=(abs((Spectroscopy_Temp)-(Spectroscopy(index_now))).^2); %(abs(abs(Spectrum_Temp)-abs(Spectrum(index_now))).^2)+
          

        [value index]=min(Merit);
        value_total_1(p)=value_total_1(p)+value;
        Spectroscopy_Check(index_now)=Spectroscopy_Temp(index);
    ...
    index_old=0;
    Spectrum_Check(1:length(Spectrum))=0;
    for j=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
        ...
        Spectrum_Temp=real((r1.*(d0.^2))+t1.*t1_r.*r2.*((d0.*d).^2))./abs(r_BK7(index_now).*(dref.^2));                       %神說是cosine
        
        Merit=(abs((Spectrum_Temp)-(Spectrum(index_now))).^2); %(abs(abs(Spectrum_Temp)-abs(Spectrum(index_now))).^2)+
          
        if range_specified == 1
            if (index_now == Frequency_Center_Index) || (index_now == (Frequency_Center_Index -1))
                range_upper=min(n_should_index+40,length(Merit));
                range_lower=max(n_should_index-40,1);
            elseif (index_now > Frequency_Center_Index) || (index_now < (Frequency_Center_Index -1))
                range_upper=min(index_old+40,length(Merit));
                range_lower=max(index_old-40,1);
            end
        ...
        Spectrum_Check(index_now)=Spectrum_Temp(index);

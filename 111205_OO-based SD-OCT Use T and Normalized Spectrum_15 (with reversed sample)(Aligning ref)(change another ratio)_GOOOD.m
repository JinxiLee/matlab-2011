...
% For Spectroscopy    
    ...
    for j=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
        ...
        Spectrum_Lower_Temp=((t1.*t1_r.*r2.*(d.^2))./r_BK7(index_now))*0.8.*exp(i.*Ratio(q));                       %神說是cosine
    ...
    for j=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
        ...
        Spectrum_Lower_Temp=((t1.*t1_r.*r2.*(d.^2))./r_BK7(index_now))*0.8.*exp(i.*Ratio(q));                       %神說是cosine
        Spectrum_Lower_Temp_n=((t1.*t1_r.*r2.*(d_n.^2))./r_BK7(index_now))*0.8.*exp(i.*Ratio(q));                       %神說是cosine
        Spectrum_Lower_Temp_k=((t1.*t1_r.*r2.*(d_k.^2))./r_BK7(index_now))*0.8.*exp(i.*Ratio(q));                       %神說是cosine        
        %Spectrum_Temp=((r1+t1.*t1_r.*r2.*(d.^2))./r_BK7(index_now));                       %神說是cosine
        Spectrum_Temp=Spectrum_Upper_Temp+Spectrum_Lower_Temp;

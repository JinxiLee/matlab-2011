...
Thickness_temp=[2.4:0.1:2.8]*1E-6;
...
n_final(1:length(Frequency),1:length(Thickness_temp))=1.5;
k_final(1:length(Frequency),1:length(Thickness_temp))=0;
Spectroscopy_Amplitude_Check(1:length(Spectrum),1:length(Thickness_temp))=0;
T_check(1:length(Spectrum),1:length(Thickness_temp))=0;

Spectrum_Check(1:length(Spectrum),1:length(Thickness_temp))=0;
Spectrum_Upper_Check(1:length(Spectrum),1:length(Thickness_temp))=0;
Spectrum_Lower_Check(1:length(Spectrum),1:length(Thickness_temp))=0;
Spectrum_Lower_Check_n(1:length(Spectrum),1:length(Thickness_temp))=0;
Spectrum_Lower_Check_k(1:length(Spectrum),1:length(Thickness_temp))=0;
for q=1:length(Thickness_temp)
Thickness=Thickness_temp(q);
...
plot(Wavelength_micron(Wavelength_micron<0.6),k_final(Wavelength_micron<0.6,10));
%plot(1:Number_of_Loop,value_total_1,1:Number_of_Loop,value_total_2);
dlmwrite('Thickness_temp.txt',Thickness_temp,'delimiter','\t','newline','pc','precision','%.12f');

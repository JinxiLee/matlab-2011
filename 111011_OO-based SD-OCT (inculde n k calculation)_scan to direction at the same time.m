...
Signal_Lower=Signal_Carrier;
    ...
    pixel_1=430;               % the end of upper (the larger peak)
    pixel_2=1000;               % the end of lower (the larger peak)
...
elseif reversed == 1
    pixel_1=700;               % the end of upper (the larger peak)
    pixel_2=7200;               % the end of lower (the larger peak)
    pixel_3=7880;               % the start of upper (the larger peak)
    Signal_Upper(pixel_1:pixel_3)=0;
    Signal_Lower(1:pixel_2)=0;
    Signal_Lower(pixel_3:end)=0;
...
n_o=(n_should-0.3):0.001:(n_should+0.3);
k_o=0:0.001:0.1;
k_o=k_o';
...
n(1:length(Frequency_Final),1:length(thickness_temp))=1.75;        %df is the same as frequency, but only take a portion of it (1000*df)
k(1:length(Frequency_Final),1:length(thickness_temp))=0;
...
for q=1:length(thickness_temp)
    ...
    for p=1:(Ending_pixel_f_considered-Starting_pixel_f_considered+1)            %start from center_frequency_index, to Ending_pixel_f, back to center_frequency_index, finally Starting_pixel_f_considered
        ...
        d=exp(i*2*pi.*Frequency_Final(index_now)/c.*(-1*n_temp+i*k_temp).*thickness_temp(q));   %注意! -1*n!
        A_temp=(d.^2).*r2./r1;
        ...
        if range_specified == 1
          if index_now == center_frequency_index
                range_upper=n_should_index+40;
                range_lower=n_should_index-40;
                [value index_1]=min(DD(:,n_should_index-40:n_should_index+40));
                [value index_2]=min(value);
                index_2_temp=index_2;
                index_2=n_should_index-40+index_2-1;
                index_2_old=index_2;
                index_2_center=index_2;
            elseif index_now > center_frequency_index
                if (index_2_old >= 41) && (index_2_old < size(DD,2)-40)
                    range_upper=index_2_old+40;
                    range_lower=index_2_old-40;
                    [value index_1]=min(DD(:,index_2_old-40:index_2_old+40));
                    [value index_2]=min(value);
                    index_2_temp=index_2;
                    index_2=index_2_old-40+index_2-1;
                    index_2_old=index_2;
                else
            ...
            elseif index_now == (center_frequency_index -1)
                if (index_2_center >= 41) && (index_2_center < size(DD,2)-40)
                    range_upper=index_2_center+40;
                    range_lower=index_2_center-40;                    
                    [value index_1]=min(DD(:,index_2_center-40:index_2_center+40));
                    [value index_2]=min(value);
                    index_2_temp=index_2;
                    index_2=index_2_center-40+index_2-1;
                    index_2_old=index_2;
                else
                    [value index_1]=min(DD);
                    [value index_2]=min(value);
...
dlmwrite('Signal_Upper_f_amp.txt',Signal_Upper_f_amp,'delimiter','\t','newline','pc');
dlmwrite('Signal_Lower_f_amp.txt',Signal_Lower_f_amp,'delimiter','\t','newline','pc');
dlmwrite('n_final.txt',n_final,'delimiter','\t','newline','pc');
dlmwrite('k_final.txt',k_final,'delimiter','\t','newline','pc');

dlmwrite('A_exp_abs.txt',A_exp_abs,'delimiter','\t','newline','pc');
...
dlmwrite('Signal_Upper.txt',Signal_Upper,'delimiter','\t','newline','pc');

dlmwrite('Signal_Lower.txt',Signal_Lower,'delimiter','\t','newline','pc');

...
%% Iteration starts
distance_variable=0.000001;

iteration=0;    % count of full iteration (to C1 and to C1)

while (iteration < 10)
    % to C1
    E_upper=Reference_f_for_Hilbert_amp.* ( ( ( ((n-1.46).^2)  +  k.^2       )  ./ ( ((n+1.46).^2)  +  n.^2       )  ).^0.5 ) .* exp(atan(2*k*1.46./((n.^2)+(k.^2)-1.46^2)));
    E_lower=Reference_f_for_Hilbert_amp.*exp(2*i*(n+i*k)*2*pi.*frequency/c*distance_variable).* ( ( ( ((n-1).^2)  +  k.^2       )  ./ ( ((k+1).^2)  +  k.^2       )  ).^0.5 ) .* exp(atan(-2*k*1./((n.^2)+(k.^2)-1^2)));
    E_phase=angle(E_upper+E_lower);                   % auto normalize the new E to the original one; means C1 cannot change its absolute amplitude
    
    E=fft(E_t_exp.*exp(angle(ifft(Reference_f_amp.*( ( ( ((n-1.46).^2)  +  k.^2       )  ./ ( ((n+1.46).^2)  +  k.^2       )  ).^0.5 ) .* exp(atan(2*k*1.46./((n.^2)+(k.^2)-1.46^2))) + Reference_f_amp.*exp(-2*(n+i*k)*distance_variable).*( ( ( ((n-1).^2)  +  k.^2       )  ./ ( ((n+1).^2)  +  k.^2       )  ).^0.5 ) .* exp(atan(-2*k*1./((n.^2)+(k.^2)-1^2)))))));
    ...
    for p=1:801            %the specified frequency range
        ...
        E_lower=Reference_f_amp(Starting_pixel_f+p-1).*exp(-2*(n_temp+i*k_temp)*distance_variable).*R_lower;
        E_temp=E_upper+E_lower-E;                   % to find the smallest E_temp

...
disatance=[0:0.02:5];    %micron
distance_calc_1(1:length(disatance))=0;
distance_calc_2(1:length(disatance))=0;

for j=1:length(disatance)
Sig1=circshift(CS_real,round(disatance(j)*1E-6/d_t/c));
Sig2=circshift(CS_real,-round(disatance(j)*1E-6/d_t/c));
...
distance_calc_1(j)=(inter_peakindex_right_1+inter_peakindex_FD-inter_peakindex_left_1-1)*d_t*c/(1E-6)/2;
distance_calc_2(j)=(inter_peakindex_right_2+inter_peakindex_FD-inter_peakindex_left_2-1)*d_t*c/(1E-6)/2;
end
...
plot(disatance,distance_calc_2,disatance,distance_calc_1);

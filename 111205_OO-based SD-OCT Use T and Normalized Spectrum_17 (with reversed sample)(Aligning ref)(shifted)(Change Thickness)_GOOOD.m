...
Thickness_Temp=[2.70:0.01:2.73]*1E-6;%[2.68:0.01:2.78]*1E-6;%[2.43:0.05:2.93]*1E-6;       %% 發現: 對T的fitting而言, thickness也有多值問題!!!
                                                       %例如此時2.73與2.88皆可對T有較
                                                       %佳fitting,
                                                       %但幸好thickness可以是已知
Position_Error=0.022E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;
n_should=1.65;
delta_n=0.08;
...
% For Spectroscopy    
    ...
    for j=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
        ...
        Spectrum_Lower_Temp=((t1.*t1_r.*r2.*(d.^2))./r_BK7(index_now))*0.75;                       %神說是cosine
    ...
    for j=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
        ...
        Spectrum_Lower_Temp=((t1.*t1_r.*r2.*(d.^2))./r_BK7(index_now))*0.75;                       %神說是cosine
        Spectrum_Lower_Temp_n=((t1.*t1_r.*r2.*(d_n.^2))./r_BK7(index_now))*0.75;                       %神說是cosine
        Spectrum_Lower_Temp_k=((t1.*t1_r.*r2.*(d_k.^2))./r_BK7(index_now))*0.75;                       %神說是cosine        
        %Spectrum_Temp=((r1+t1.*t1_r.*r2.*(d.^2))./r_BK7(index_now));                       %神說是cosine
        Spectrum_Temp=(Spectrum_Upper_Temp+Spectrum_Lower_Temp)*Spectrum_Additional(index_now);

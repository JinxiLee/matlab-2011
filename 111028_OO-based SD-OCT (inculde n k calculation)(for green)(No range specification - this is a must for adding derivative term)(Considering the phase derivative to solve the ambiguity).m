...
%What we need is: taking the derivative of A, and then: 1. abs 2. angle
A_exp_diff(1:length(A_exp))=0;
A_exp_diff(2:end)=diff(A_exp);


A_exp_diff_abs=abs(A_exp_diff);
A_exp_diff_phase=angle(A_exp_diff);
...
A_exp_diff_abs(isnan(A_exp_diff_abs))=0;
A_exp_diff_phase(isnan(A_exp_diff_phase))=0;
...
for q=1:length(n_should)
    ...
    for p=1:(Ending_pixel_f_considered-Starting_pixel_f_considered+1)            %start from center_frequency_index, to Ending_pixel_f, back to center_frequency_index, finally Starting_pixel_f_considered
        ...
        d=exp(i*2*pi.*Frequency_Final(index_now)/c.*(-1*n_temp+i*k_temp).*thickness_temp);   %注意! -1*n! 這會不會造成什麼問題?
        A_temp=(d.^2).*r2./r1;
        A_temp_abs=abs(A_temp);
        A_temp_phase=angle(A_temp);
        A_temp_diff_abs=abs(A_temp-A_last);
        A_temp_diff_phase=angle(A_temp-A_last);
        DD=((A_temp_abs-A_exp_abs(index_now)).^2)+((A_temp_phase-A_exp_phase(index_now)).^2)+1E10*((A_temp_diff_abs-A_exp_diff_abs(index_now)).^2)+1E10*((A_temp_diff_phase-A_exp_diff_phase(index_now)).^2);
        ...
        A_last=A_temp(index_1(index_2), index_2);
    end
    k_max_index=max(k(:,q));
    value_total=abs(n(center_frequency_index,q)*thickness_temp-OPD);

...
Is_o=importdata('D:\110426_Original PSOCT data\PALC_DC_high Z_10V_1_s.txt');
...
Ip_o=importdata('D:\110426_Original PSOCT data\PALC_DC_high Z_10V_1_p.txt');
...
tempMaxs=max(Is(1:2648));

tempMaxp=max(Ip(1:2648));

for ratio=0.1:0.01:1    

Is_sub=Is-CS_real/max(CS_real)*Is_peakvalue*ratio;  %this 0.9 can be critical...
if max(Is_sub(1:2648))<tempMaxs
    tempMaxs=max(Is_sub(1:2648));
    goodRatios=ratio;
end
end

for ratio=0.1:0.01:2    

Ip_sub=Ip-CS_real/max(CS_real)*Ip_peakvalue*ratio;  %this 0.9 can be critical...
if max(Ip_sub(1:2648))<tempMaxp
    tempMaxp=max(Ip_sub(1:2648));
    goodRatiop=ratio;
end
end
...
Ip_sub=Ip-CS_real/max(CS_real)*Ip_peakvalue*goodRatiop;


%plot(position,Is_filtered,position,Ip_filtered);

plot(position,Is,position,Ip,position,real(Is_sub),position,real(Ip_sub)); figure(gcf)
...
%dlmwrite('D:\Is_sub.txt',Is_sub,'delimiter','\t','newline','pc');
%dlmwrite('D:\Ip_sub.txt',Ip_sub,'delimiter','\t','newline','pc');
...
plot(position,phase,position,Ip*10000000);

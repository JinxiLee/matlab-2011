...
Data=importdata('111031_New Red (400mA 3ms 100ave) at c.txt');
...
Signal_Envelope_new=Signal_Envelope;
Signal_Envelope_new(1:200)=0;
Signal_Envelope_new((length(Signal_Envelope_new)-200):end)=0;

[maxvalue maxindex_2]=max(Signal_Envelope_new,[],1);

peak_2_position=Position_micron(maxindex_2);

if maxindex_2<length(Signal_Envelope_new)/2
    thickness=abs(peak_1_position-peak_2_position);
else
    thickness=abs(Position_micron(length(Signal_Envelope_new))-peak_2_position);
end
%% ROI

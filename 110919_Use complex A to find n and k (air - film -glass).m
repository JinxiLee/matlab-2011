...
thickness_temp=3.5E-6;             %0.0000020:0.0000001:0.000003;
...
for q=1:length(thickness_temp)
    ...
    for p=1:801            %the specified frequency range  
        d=exp(i*2*pi.*frequency(Starting_pixel_f+p-1)/c.*(-1*n_temp+i*k_temp).*thickness_temp(q));   %注意! -1*n!
        A_temp=(d.^2).*r2./r1;
        ...
        %else
            ...
            index_2_temp=index_2;
        %end
        n(Starting_pixel_f+p-1,q)=n_temp(index_1(index_2_temp), index_2);    %index1,index2=k,n
        k(Starting_pixel_f+p-1,q)=k_temp(index_1(index_2_temp), index_2);

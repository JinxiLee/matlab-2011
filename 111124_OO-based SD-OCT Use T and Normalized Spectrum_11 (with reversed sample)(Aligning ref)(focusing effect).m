...
n_final(1:length(Frequency),1:length(Ratio_Focusing))=1.5;
k_final(1:length(Frequency),1:length(Ratio_Focusing))=0;
Spectroscopy_Amplitude_Check(1:length(Spectrum),1:length(Ratio_Focusing))=0;
T_check(1:length(Spectrum),1:length(Ratio_Focusing))=0;

Spectrum_Check(1:length(Spectrum),1:length(Ratio_Focusing))=0;
Spectrum_Upper_Check(1:length(Spectrum),1:length(Ratio_Focusing))=0;
Spectrum_Lower_Check(1:length(Spectrum),1:length(Ratio_Focusing))=0;
Spectrum_Lower_Check_n(1:length(Spectrum),1:length(Ratio_Focusing))=0;
Spectrum_Lower_Check_k(1:length(Spectrum),1:length(Ratio_Focusing))=0;
for q=1:length(Ratio_Focusing)
...
% For Spectroscopy    
    ...
    for j=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
        ...
        Spectrum_Upper_Temp=((r1)./r_BK7(index_now)*Ratio_Focusing(q));                       %神說是cosine
        
        Spectrum_Lower_Temp=((t1.*t1_r.*r2.*(d.^2))./r_BK7(index_now)*Ratio_Focusing(q));                       %神說是cosine
    ...
    for j=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
        ...
        Spectrum_Lower_Temp=((t1.*t1_r.*r2.*(d.^2))./r_BK7(index_now)*Ratio_Focusing(q));                       %神說是cosine
        Spectrum_Lower_Temp_n=((t1.*t1_r.*r2.*(d_n.^2))./r_BK7(index_now));                       %神說是cosine
        Spectrum_Lower_Temp_k=((t1.*t1_r.*r2.*(d_k.^2))./r_BK7(index_now));                       %神說是cosine        
        %Spectrum_Temp=((r1+t1.*t1_r.*r2.*(d.^2))./r_BK7(index_now));                       %神說是cosine
        Spectrum_Temp=Spectrum_Upper_Temp+Spectrum_Lower_Temp;

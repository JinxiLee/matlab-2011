...
n_temp=1.70:0.001:1.80;
k_temp=-0.1:0.0005:0.1;
k_temp=k_temp';
...
k_temp_empty=k_temp;
k_temp_empty(:)=1;
n_temp=k_temp_empty*n_temp;
k_temp=k_temp*n_temp_empty;

%% Theoritical template

distance_variable=0.0000038;   % fixed now
R_upper=( ( ( ((n_temp-1.46).^2)  +  k_temp.^2       )  ./ ( ((n_temp+1.46).^2)  +  k_temp.^2       )  ).^0.5 ) .* exp(i*atan(2*k_temp*1.46./((n_temp.^2)+(k_temp.^2)-1.46^2)));
R_lower=( ( ( ((n_temp-1).^2)  +  k_temp.^2       )  ./ ( ((n_temp+1).^2)  +  k_temp.^2       )  ).^0.5 ) .* exp(i*atan(-2*k_temp*1./((n_temp.^2)+(k_temp.^2)-1^2)));

%% Iteration starts

%E_check=fft(Reference_f_for_Hilbert_amp.* ( ( ( ((n-1.46).^2)  +  k.^2       )  ./ ( ((n+1.46).^2)  +  n.^2       )  ).^0.5 ) .* exp(i*atan(2*k*1.46./((n.^2)+(k.^2)-1.46^2)))+Reference_f_for_Hilbert_amp.*exp(2*i*(n+i*k)*2*pi.*frequency/c*distance_variable).* ( ( ( ((n-1).^2)  +  k.^2       )  ./ ( ((k+1).^2)  +  k.^2       )  ).^0.5 ) .* exp(i*atan(-2*k*1./((n.^2)+(k.^2)-1^2))));                   % auto normalize the new E to the original one; means C1 cannot change its absolute amplitude

iteration=0;    % count of full iteration (to C1 and to C1)

while (iteration < 10)
    % to C1
    E=ifft(E_t_exp*max(abs(fft(Reference_f_for_Hilbert_amp.* ( ( ( ((n-1.46).^2)  +  k.^2       )  ./ ( ((n+1.46).^2)  +  n.^2       )  ).^0.5 ) .* exp(i*atan(2*k*1.46./((n.^2)+(k.^2)-1.46^2)))+Reference_f_for_Hilbert_amp.*exp(2*i*(n+i*k)*2*pi.*frequency/c*distance_variable).* ( ( ( ((n-1).^2)  +  k.^2       )  ./ ( ((k+1).^2)  +  k.^2       )  ).^0.5 ) .* exp(i*atan(-2*k*1./((n.^2)+(k.^2)-1^2)))))).*exp(i*angle(fft(Reference_f_for_Hilbert_amp.* ( ( ( ((n-1.46).^2)  +  k.^2       )  ./ ( ((n+1.46).^2)  +  n.^2       )  ).^0.5 ) .* exp(i*atan(2*k*1.46./((n.^2)+(k.^2)-1.46^2)))+Reference_f_for_Hilbert_amp.*exp(2*i*(n+i*k)*2*pi.*frequency/c*distance_variable).* ( ( ( ((n-1).^2)  +  k.^2       )  ./ ( ((k+1).^2)  +  k.^2       )  ).^0.5 ) .* exp(i*atan(-2*k*1./((n.^2)+(k.^2)-1^2)))))));                   % auto normalize the new E to the original one; means C1 cannot change its absolute amplitude
    % to C2
    difp(1:801)=0;
    nindexp(1:801)=0;
    for p=1:801            %the specified frequency range
        E_upper=Reference_f_for_Hilbert_amp(Starting_pixel_f+p-1)*R_upper;
        E_lower=Reference_f_for_Hilbert_amp(Starting_pixel_f+p-1)*exp(2*2*pi*i*frequency(Starting_pixel_f+p-1)/c.*(n_temp+i*k_temp).*distance_variable).*R_lower;
        E_temp=abs(E_upper+E_lower-E(Starting_pixel_f+p-1));                   % to find the smallest E_temp
        %E_temp=abs(abs(E_upper+E_lower)-abs(E(Starting_pixel_f+p-1)));
        [value index_1]=min(E_temp);
        [value index_2]=min(value);
        n(Starting_pixel_f+p-1)=n_temp(index_1(index_2), index_2);
        k(Starting_pixel_f+p-1)=k_temp(index_1(index_2), index_2);
    end
    iteration=iteration+1;
    E_check=abs(fft(Reference_f_for_Hilbert_amp.* ( ( ( ((n-1.46).^2)  +  k.^2       )  ./ ( ((n+1.46).^2)  +  n.^2       )  ).^0.5 ) .* exp(i*atan(2*k*1.46./((n.^2)+(k.^2)-1.46^2)))+Reference_f_for_Hilbert_amp.*exp(2*i*(n+i*k)*2*pi.*frequency/c*distance_variable).* ( ( ( ((n-1).^2)  +  k.^2       )  ./ ( ((k+1).^2)  +  k.^2       )  ).^0.5 ) .* exp(i*atan(-2*k*1./((n.^2)+(k.^2)-1^2)))));                   % auto normalize the new E to the original one; means C1 cannot change its absolute amplitude
    errori(iteration)=sum(abs(E_check-E_t_exp*max(E_check)).^2);
end

%% Used for checking

%plot(position,circshift(E_check,300),position,circshift(E_t_exp*max(E_check),300));
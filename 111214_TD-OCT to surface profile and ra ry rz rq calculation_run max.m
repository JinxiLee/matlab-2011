...
data_Bscan=importdata('D:\111214\2Hz 3V ra1.8 1000 1micron new 2.txt');
...
mins=0;
    for j=1:size(profile,1)
        profile_tilted(j)=profile(j)-lateral_position(j)*tan(angle(q)*pi/180);
    end
    profile_sub(:,q)=profile_tilted-mean(profile_tilted);
    
    for j=1:size(profile_sub,1)
        if j+19 < size(profile_sub,1)
            profile_sub(j)=max(profile_sub(j:j+19));
        else
            profile_sub(j)=max(profile_sub(j:end));
        end
    end
    
    profile_sub(:,q)=profile_sub-mean(profile_sub);
    
    Ra(q)=sum(abs(profile_sub(:,q)))/size(profile_sub(:,q),1);
    Rq(q)=(sum((profile_sub(:,q)).^2)/size(profile_sub(:,q),1)).^0.5;
    Ry(q)=max(profile_sub(:,q))-min(profile_sub(:,q));
    %Rz
    ...
    if size(maxs_sorted,2)>=5 && size(mins_sorted,2)>=5
        Rz(q)=(mean(maxs_sorted(1:5))+abs(mean(mins_sorted(1:5))));
    else
        Rz(q)=99999999;
    end
end

plot(lateral_position,profile,lateral_position,profile_tilted);

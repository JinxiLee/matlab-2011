...
thickness_temp=2.380977485484966E-6;
n_should=1.6:0.1:2;

Starting_pixel_f_considered=230;
Ending_pixel_f_considered=300;
...
DC_cutoff=1500;                 %in TD

%% Data Loading

cd('D:\');
%Data=importdata('111010_Green (2500microsec) no word 5 ave 100.txt');
Data=importdata('111031_New Red (400mA 3ms 100ave rev and with dis comp) near c.txt');
...
pixel_1=300;               % the end of upper (the larger peak)
pixel_2=2200;               % the end of lower (the larger peak)
pixel_3=7200;               % the start of upper (the larger peak)
...
A_exp_phase_diff(1:length(A_exp_phase),1)=0;
A_exp_phase_diff(2:end,1)=diff(A_exp_phase);
...
A_exp_phase_diff(isnan(A_exp_phase_diff))=0;
...
for q=1:length(n_should)
    ...
    for p=1:(Ending_pixel_f_considered-Starting_pixel_f_considered+1)            %start from center_frequency_index, to Ending_pixel_f, back to center_frequency_index, finally Starting_pixel_f_considered
        ...
        t1=2*(n1(index_now))./(n_temp+n1(index_now)+i*k_temp);
    ...
    n_diff(1:length(n(:,q)),q)=0;
    n_diff(2:end,q)=diff(n(:,q));
    A_phase_diff(:,q)=4*pi/c*thickness_temp.*(n(:,q)+Frequency_Final.*n_diff(:,q));
    value_total(q)=sum((A_phase_diff(:,q)-A_exp_phase_diff).^2);
    
    if value_total(q) < value_temp
        finalindex=q;                % the same value for all wavelength!!
        value_temp=value_total(q);
    end
end
...
%plot(Wavelength_micron(Wavelength_micron<1),abs(A_exp_diff(Wavelength_micron<1)),Wavelength_micron(Wavelength_micron<1),abs(A_diff(Wavelength_micron<1)));
...
%plot(Wavelength_micron(Wavelength_micron<1),abs(A_exp_diff(Wavelength_micron<1)),Wavelength_micron(Wavelength_micron<1),abs(A_diff(Wavelength_micron<1)));
plot(Wavelength_micron(Wavelength_micron<1),A_exp_phase_diff(Wavelength_micron<1,:),Wavelength_micron(Wavelength_micron<1),A_phase_diff(Wavelength_micron<1,:));
